...
面向 **OpenCode 2.0.9** 的 token 用量插件，当前版本 **0.1.0**。在原生 TUI 侧边栏追加用量面板，统计当前会话所在的整棵会话树，包括未打开的子代理。

## 预览

```text
Token Usage
Session tree · estimated cost
Input                 54.2K
Output                 6.8K
Reasoning              4.1K
Cache Read            31.7K
...
需要 Node.js 22+、npm 和 OpenCode **2.0.9**。本版本没有发布到 npm；更高版本的 OpenCode 尚未验证。
...
然后在目标项目运行 `opencode`，进入会话。OpenCode 自动加载主入口及 `./tui` 入口；包根目录还提供 `index.js` 和 `tui.js`，适配 2.0.9 的本地目录发现方式。面板追加到 `sidebar.content`，宿主原有侧边栏内容继续保留。侧边栏的显示与宽度由宿主管理；终端需要足够宽，并且 CLI 设置 `session.sidebar` 为 `auto`。

OpenCode 2.0.9 在子代理视图中强制隐藏侧边栏，因此插件通过 `session.composer.top` 在输入区上方显示同一完整面板。进入子代理仍统计整棵会话树，费用改用该子代理的活动模型。
...
- 所有统计行始终保留。空会话显示零；首次读取显示 `Loading…` 和 `—`，首次失败显示 `Unavailable`。刷新失败保留上次完整结果，标注 `Not updated` 并自动重试。
...
没有模型价格或适用档位时按 OpenCode 官方计费实现回退为 **0**，并显示 `Default price: missing = $0`。合法的免费模型不会被标记为缺失价格；价格请求失败会显示未更新状态。这里的“默认价格”并非内置的公开价格目录。

数字使用 K/M，最多一位小数并去掉尾部 `.0`；例如 `1200 → 1.2K`、`125000 → 125K`、`1500000 → 1.5M`。美元保留两位小数，正数不足一美分显示 `<$0.01`。
...
`test:smoke` 需要本机 OpenCode 2.0.9、Python 3、可用的本地端口及 npm 网络访问。它打包并安装真实产物，在临时目录启动隔离的 OpenCode 服务和真实 TUI，通过本地模拟提供商检查加载、空会话、消息完成后的刷新、真实子代理累计及模型切换。不会修改现有 OpenCode 配置或使用付费模型。终端捕获和测试记录保留在输出的临时路径。

2026-09-19 已通过类型检查、17 项自动化测试、构建和上述真实集成验证，详见 [验证记录](docs/verification.md)。
